...
class GalleryEngine {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const galleryItems =
      items?.length ? items : [{ image: "/assets/launch/entry-cube-a1.png", text: "Portfolio" }];
    this.galleryItems = galleryItems;
    this.mediasImages = galleryItems.length < 8 ? galleryItems.concat(galleryItems) : galleryItems;
  ...
  onTouchDown(event) {
    ...
    this.pointerStart = { x: point.clientX, y: point.clientY };
  ...
  onTouchMove(event) {
    ...
    if (this.pointerStart && Math.abs(this.pointerStart.x - x) > 8) {
  ...
  onClick(event) {
    if (this.hasDragged || typeof this.onItemClick !== "function") return;
    this.openClosestMedia(event.clientX, event.clientY);
  }

  openClosestMedia(clientX, clientY) {
    ...
    this.medias.forEach((media) => {
      const screenX = (media.plane.position.x / this.viewport.width + 0.5) * this.screen.width;
      const screenY = (0.5 - media.plane.position.y / this.viewport.height) * this.screen.height;
      const distance = Math.abs(screenX - x) + Math.abs(screenY - y) * 0.25;
  ...
  update(time = 0) {
    ...
    if (
      ...
      time - this.lastRenderTime < this.frameInterval
    ) {
      return;
    }
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="环形作品画廊，可拖动或使用左右方向键浏览"
    />
  );
}
